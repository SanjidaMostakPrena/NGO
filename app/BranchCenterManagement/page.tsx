...
const BranchCenterManagement = () => {
  ...
  const stats = [
    { label: 'মোট শাখা', count: '২৪টি', growth: '+২.৫%', icon: <Building2 size={20} />, iconBg: 'bg-indigo-50 text-[#0c2483]' },
    { label: 'সক্রিয় কেন্দ্র', count: '১৫৬টি', growth: '+৫.০%', icon: <Network size={20} />, iconBg: 'bg-orange-50 text-orange-600' },
    { label: 'মোট সদস্য', count: '১২,৫৪০ জন', growth: '+১.৮%', icon: <Users size={20} />, iconBg: 'bg-emerald-50 text-emerald-600' },
    { label: 'মিটিং শিডিউল', count: '১৮টি', tag: 'আজকের সভা', icon: <Calendar size={20} />, iconBg: 'bg-red-50 text-red-500' },
  ];

  // Mock Data for Upcoming Meetings (Extended for Dynamic View)
  const baseMeetings = [
    { date: '২৪', month: 'MAY', title: 'সাপ্তাহিক কিস্তি আদায় সভা', location: 'মিরপুর শাখা-কেন্দ্র ০১', time: 'সকাল ১০:৩০', members: '১৫ জন', dateBg: 'bg-[#002bb8]' },
    { date: '২৬', month: 'MAY', title: 'নতুন ঋণ বিতরণ আলোচনা', location: 'উত্তরা শাখা-কেন্দ্র ০৫', time: 'দুপুর ০২:১৫', members: '০৮ জন', dateBg: 'bg-amber-950' },
    { date: '২৭', month: 'MAY', title: 'মাসিক সঞ্চয় রিভিউ', location: 'বাড্ডা শাখা-কেন্দ্র ১৩', time: 'সকাল ০৯:০০', members: '২৫ জন', dateBg: 'bg-emerald-800' },
  ];

  const extendedMeetings = [
    ...baseMeetings,
    { date: '২৯', month: 'MAY', title: 'বিশেষ অডিট ও ট্র্যাকিং', location: 'ধানমন্ডি শাখা-কেন্দ্র ০৪', time: 'সকাল ১১:০০', members: '১২ জন', dateBg: 'bg-indigo-950' },
    { date: '৩০', month: 'MAY', title: 'নতুন সদস্য ওরিয়েন্টেশন', location: 'গুলশান শাখা-কেন্দ্র ০২', time: 'বিকাল ০৩:৩০', members: '৩০ জন', dateBg: 'bg-violet-800' },
    { date: '০২', month: 'JUN', title: 'ত্রৈমাসিক সমন্বয় সভা', location: 'প্রধান কার্যালয় প্যানেল', time: 'সকাল ১০:০০', members: '৫০ জন', dateBg: 'bg-rose-900' },
  ];

  const upcomingMeetings = showAllMeetings ? extendedMeetings : baseMeetings;

  // Mock Data for Centers List (Extended for Dynamic View)
  const baseCenters = [
    { name: 'শাপলা কেন্দ্র - ০১', id: 'CEN-001', branch: 'মিরপুর শাখা', members: '৪৫', status: 'সক্রিয়', statusType: 'active' },
    { name: 'জুঁই কেন্দ্র - ০৫', id: 'CEN-024', branch: 'উত্তরা শাখা', members: '৩২', status: 'সক্রিয়', statusType: 'active' },
    { name: 'গোলাপ কেন্দ্র - ১২', id: 'CEN-089', branch: 'সাভার শাখা', members: '৫০', status: 'বিশ্রাম', statusType: 'inactive' },
  ];

  const extendedCenters = [
    ...baseCenters,
    { name: 'রজনীগন্ধা কেন্দ্র - ০৩', id: 'CEN-012', branch: 'ধানমন্ডি শাখা', members: '২৮', status: 'সক্রিয়', statusType: 'active' },
    { name: 'সূর্যমুখী কেন্দ্র - ০৮', id: 'CEN-055', branch: 'বাড্ডা শাখা', members: '৩৬', status: 'সক্রিয়', statusType: 'active' },
    { name: 'পদ্ম কেন্দ্র - ১৫', id: 'CEN-104', branch: 'মোহাম্মদপুর শাখা', members: '৪২', status: 'বিশ্রাম', statusType: 'inactive' },
  ];

  const centersList = showAllCenters ? extendedCenters : baseCenters;
    ...
    <div className="bg-secondary min-h-screen p-6 font-sans text-slate-800 relative">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-8 gap-4">
        <div>
          ...
          <p className="text-slate-500 text-sm">
            আঞ্চলিক কার্যালয় সমূহের বিস্তারিত তথ্য ও কেন্দ্র সমন্বয় প্যানেল
          </p>
        </div>
        <button 
          onClick={() => setIsModalOpen(true)}
          className="flex items-center gap-2 bg-[#002bb8] hover:bg-[#002294] text-white px-5 py-2.5 rounded-xl font-medium text-sm transition-all shadow-sm shrink-0"
        >
          <Plus size={16} />
          <span>নতুন শাখা যোগ করুন</span>
        </button>
        ...
        <div className="bg-white rounded-2xl border border-slate-100 shadow-sm flex flex-col justify-between overflow-hidden">
          ...
          <div className="bg-indigo-50/50 border-t border-slate-100 text-center py-3.5">
            <button 
              ...
              className="text-[#002bb8] hover:text-[#002294] text-sm font-semibold transition-all inline-flex items-center gap-1"
            >
              <span>{showAllMeetings ? 'কমিয়ে দেখুন' : 'সম্পূর্ণ শিডিউল দেখুন'}</span>
              {showAllMeetings ? <ChevronUp size={14} /> : <ChevronRight size={14} />}
            </button>
          </div>
        </div>
        ...
        <div className="lg:col-span-2 bg-white rounded-2xl border border-slate-100 shadow-sm p-6">
          <div className="flex justify-between items-center mb-5">
            ...
            <button 
              ...
              className="text-[#002bb8] hover:text-[#002294] text-xs font-semibold flex items-center gap-0.5 transition-all"
            >
              <span>{showAllCenters ? 'সংক্ষিপ্ত করুন' : 'বিস্তারিত দেখুন'}</span>
              {showAllCenters ? <ChevronUp size={14} /> : <ChevronRight size={14} />}
            </button>
          </div>
      ...
      {isModalOpen && (
        <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4 animate-fade-in">
          <div className="bg-white rounded-2xl w-full max-w-md p-6 shadow-2xl relative border border-slate-100">
            ...
            <div className="mb-4">
              <h3 className="text-xl font-bold text-[#0c2483]">নতুন শাখা যুক্ত করুন</h3>
              <p className="text-slate-400 text-xs">সিস্টেমে একটি নতুন আঞ্চলিক শাখা এন্ট্রি করুন</p>
            </div>
            
            <form onSubmit={(e) => { e.preventDefault(); setIsModalOpen(false); }} className="space-y-3">
              <div>
                <label className="text-xs font-semibold text-slate-600 block mb-1">শাখার নাম</label>
                <input type="text" placeholder="উদা: ধানমন্ডি শাখা" className="w-full text-sm px-3 py-2 border border-slate-200 rounded-xl focus:outline-none focus:border-[#002bb8]" required />
              </div>
              <div>
                <label className="text-xs font-semibold text-slate-600 block mb-1">অঞ্চল / লোকেশন</label>
                <input type="text" placeholder="উদা: ঢাকা পশ্চিম" className="w-full text-sm px-3 py-2 border border-slate-200 rounded-xl focus:outline-none focus:border-[#002bb8]" required />
              </div>
              <div className="flex gap-2 pt-3">
                <button type="button" onClick={() => setIsModalOpen(false)} className="flex-1 bg-slate-100 text-slate-700 text-sm font-semibold py-2 rounded-xl hover:bg-slate-200 transition-colors">বাতিল</button>
                <button type="submit" className="flex-1 bg-[#002bb8] text-white text-sm font-semibold py-2 rounded-xl hover:bg-[#002294] transition-colors">সংরক্ষণ করুন</button>
              </div>
            </form>
          </div>
        </div>
